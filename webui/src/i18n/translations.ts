...
Note: Once enabled, requests without authentication information are rejected instead of being treated as Default platform requests.`,
  ...
  "单 IP 最大账号数": "Max accounts per IP",
  "单 IP 最大账号数（可选）": "Max accounts per IP (optional)",
  "每 IP 最大账号数必须是不小于 0 的整数": "Max accounts per IP must be an integer >= 0",
  "限制同一出口 IP 在统计窗口内最多绑定的去重账号数，留空或 0 表示不限制。":
    "Limit distinct accounts bound to the same egress IP within the window. Empty or 0 means unlimited.",
  "账号统计窗口": "Account counting window",
  "账号统计窗口（可选）": "Account counting window (optional)",
  "统计窗口必须是正的时长，例如 2h、90m 或 1h30m":
    "Counting window must be a positive duration, e.g. 2h, 90m or 1h30m",
  "账号数按此时长滑动窗口统计，仅在启用单 IP 限制时生效。":
    "Accounts are counted in a sliding window of this length; only effective when the per-IP limit is enabled.",
  "IP 账号配额": "IP Account Quota",
  "各出口 IP 在统计窗口内的去重账号数": "Distinct accounts per egress IP within the counting window",
  "未启用：在“配置”页设置单 IP 最大账号数后生效":
    "Disabled: set max accounts per IP on the Config tab to enable",
  "单 IP 上限": "Per-IP limit",
  "统计窗口": "Counting window",
  "满额 IP 数": "Full IPs",
  "配额阻断": "Quota blocked",
  "兜底放行": "Fallback admitted",
  "窗口内账号数": "Accounts in window",
  "使用率": "Usage",
  "满额": "Full",
  "仅展示账号数最多的前 {{count}} 个 IP": "Showing top {{count}} IPs by account count",
  "统计窗口内暂无账号绑定记录": "No account bindings recorded in the counting window",
  "IP 配额": "IP quota",
  " 账号": " accounts",
  "未启用": "Disabled",
